# for the GIR fixed mode, in M3 and DGNF configuration only
# calculate how many reconfigurations are needed
#
# assumptions:
# - select asterims based purely on performance, not on how long they are valid
# - reconfigure when we have to, not when a better asterism is available
#
# method:
# - load the previously computed asterisms for GIR tracking mode
# - calculate how long we can hold the asterism on the probes
# - recalculate the optimal asterims
# - etc.
# save all asterisms
...
def select_guidestars(rotation_angle):
    # subroutine to select guide stars at a different rotation angle
    # based on dgnfguidestars.py
    ...
    gspos = rotate(np.transpose(np.array([xposarcsec,yposarcsec])),rotation_angle)
    ...
    nguidestars = gspos.shape[0]
    if n_probes_with_stars >= 2 and nguidestars >= 2:
...
for fieldno in range(953,954):
    
    telzen = (90-telel[fieldno])*np.pi/180.
    airmass = 1./np.cos(telzen)fa
    ...
    cfg['Target']['rotator angle'][0] = 10 # rotator angle in degrees
    ...
    inputfilename = os.path.expanduser('~/')+f'PYTHON/AGWS/Fields/{config}_asterism_{fieldno:04d}.csv'
    outputfilename = os.path.expanduser('~/')+f'PYTHON/AGWS/Fields/{config}_fixed_asterism_{fieldno:04d}.csv'

    # load the saved asterism 
    df = pd.read_csv(inputfilename)

    probefunction = df['Probe function'].values
    vismag = df['Visible magnitude'].values
    validprobes = np.where(vismag != 0)[0]
    xpos = (df['xpos'].values)[validprobes]
    ypos = (df['ypos'].values)[validprobes]    
    ...
    [minangle,maxangle] = range_of_motion(validator,gspos,0)
    df['configuration number'] = reconfig
    df['min angle'] = minangle
    df['max angle'] = maxangle

    df.to_csv(outputfilename, index = False, header=True, mode='w')
    
    while maxangle < 60:
        print(f"maximum angle is {maxangle}, less than 60")
        reconfig += 1
        startangle = maxangle+1
        df = select_guidestars(startangle)
        wv = np.where(df['Visible magnitude'])[0]
        
        xposarcsec = df['xpos'][wv]
        yposarcsec = df['ypos'][wv]
        gspos = np.transpose(np.array([xposarcsec,yposarcsec]))
        
        [minangle,maxangle] = range_of_motion(validator,gspos,startangle)
